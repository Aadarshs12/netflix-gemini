...
const GeminiSearchBar = () => {
  ...
  const searchTMDBMovie = async (movie) => {
    try {
      ...
      if (!data.ok) throw new Error(`TMDB HTTP ${data.status}`);
      const json = await data.json();
      return json.results || [];
    } catch (err) {
      console.error("TMDB fetch error for:", movie, err);
  ...
  const onSubmit = async (data) => {
    ...
    // Reset form immediately so user can type new query right away
    reset();
    ...
    try {
      ...
      if (movieArray.length === 0) {
        setResponse("No valid movies found in the response. Try again!");
        return;
      }
  ...
  return (
    ...
    >
      <div className="p-8 lg:w-2/5 2xl:w-1/3 md:w-6/12 w-full rounded-lg bg-black bg-opacity-80 absolute z-10 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <h1
            className="text-3xl md:text-4xl font-bold mb-8 text-transparent bg-clip-text text-center"
            style={{
              backgroundImage:
                "linear-gradient(to right, #4992e8 0%, #4e89ee 30%, #7e7dd3 60%, #ca6676 90%)",
            }}
          ...
          <div>
            <input
              className="w-full px-5 py-3 text-lg bg-gray-800 text-white border border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent placeholder-gray-400 disabled:opacity-50 transition-all"
              placeholder={
                lang[selectedLanguage]?.geminiSearchPlaceholder ||
                "e.g., Sci-fi movies with AI robots, 90s action thrillers, best Indian historical dramas…"
              }
              {...register("prompt", {
                required: lang[selectedLanguage]?.required || "This field is required",
                minLength: {
                  value: 3,
          ...
          <button
            ...
            className={`w-full py-3 px-6 rounded-xl font-semibold text-lg transition-all duration-200 flex items-center justify-center gap-3
              ${
                isLoading
                  ? "bg-purple-900 cursor-not-allowed opacity-70"
                  : "bg-purple-700 hover:bg-purple-600 active:bg-purple-800 shadow-lg hover:shadow-purple-500/30 hover:scale-[1.02]"
              } text-white`}
          >
            {isLoading ? (
              <>
        ...
        {response && (
          <div className="mt-6 p-3 bg-gray-900/90 rounded-xl border border-purple-500/20 text-white shadow-xl">
            <h2 className="text-2xl font-semibold mb-1 text-purple-300">
              Recommended Movies:
            </h2>
